...
// Vérifie que l'utilisateur est connecté et est un propriétaire
...
if (!isset($_GET['id'])) {
    header("Location: proprietaire_dash.php");
    exit();
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
	...
	$addresse = filter_input(INPUT_POST, 'addresse', FILTER_SANITIZE_SPECIAL_CHARS);
...
$numero_garage = filter_input(INPUT_POST, 'numero_garage', FILTER_SANITIZE_SPECIAL_CHARS);
$numero_lot = filter_input(INPUT_POST, 'numero_lot', FILTER_SANITIZE_SPECIAL_CHARS);
...
$syndic = filter_input(INPUT_POST, 'syndic', FILTER_SANITIZE_SPECIAL_CHARS);
$reference = filter_input(INPUT_POST, 'reference', FILTER_SANITIZE_SPECIAL_CHARS);
    ...
    if (empty($message)) {
        try {
            $stmt = $db->prepare("INSERT INTO garages (proprietaire_id, addresse, complement, CP, ville, pays, numero_garage, numero_lot, loyer_hors_charge, charge, caution, surface, commentaire, syndic, reference, piece_jointe_path, piece_jointe_nom)
                                  VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,?)");
...
<body>
    <div class="container">
        ...
        <?php endif; ?>
        <form action="dupliquer_garage.php?id=<?php echo $garage_id; ?>" method="post" enctype="multipart/form-data">
