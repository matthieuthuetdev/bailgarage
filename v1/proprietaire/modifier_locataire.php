...
// Récupérer l'ID du locataire depuis la requête GET
$locataire_id = isset($_GET['id']) ? (int)$_GET['id'] : 0;

// Récupérer les informations du locataire si l'ID est valide
$locataire = null;
if ($locataire_id > 0) {
    $stmt = $db->prepare("SELECT * FROM locataires WHERE locataire_id = ? AND proprietaire_id = ?");
    $stmt->execute([$locataire_id, $proprietaire_id]);
    $locataire = $stmt->fetch(PDO::FETCH_ASSOC);
    $stmt->closeCursor();

    if (!$locataire) {
        $error_message = "Erreur: Locataire non trouvé.";
    }
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $addresse = filter_var(trim($_POST['addresse']), FILTER_SANITIZE_STRING);
    $complement = filter_var(trim($_POST['complement']), FILTER_SANITIZE_STRING);
    ...
    $fixe = filter_var(trim($_POST['fixe']), FILTER_SANITIZE_STRING);
    ...
    if (empty($error_message)) {
        try {
            ...
            // Mise à jour des données du locataire
            $stmt = $db->prepare("UPDATE locataires SET nom = ?, prenom = ?, addresse = ?, complement = ?, CP = ?, ville = ?, telephone = ?, fixe = ?, email = ?, rgpd = ?, quittance = ?, genre = ? WHERE locataire_id = ? AND proprietaire_id = ?");
            $stmt->execute([$nom, $prenom, $addresse, $complement, $cp, $ville, $telephone, $fixe, $email, $rgpd, $quittance, $genre, $locataire_id, $proprietaire_id]);

            if ($stmt->rowCount() > 0 || !empty($_FILES['pj']['tmp_name'])) {
                ...
                if ($piece_jointe_nom) {
                    ...
                    if (!move_uploaded_file($piece_jointe_tmp, $piece_jointe_path)) {
                        ...
                        $stmt_update = $db->prepare("UPDATE locataires SET piece_jointe_path = ?, piece_jointe_nom = ? WHERE locataire_id = ? AND proprietaire_id = ?");
                        $stmt_update->execute([$piece_jointe_path, $piece_jointe_nom, $locataire_id, $proprietaire_id]);
                    }
                }

                $success_message = "Les informations du locataire ont été mises à jour avec succès.";
                $db->commit(); // Valider la transaction
            } else {
                $error_message = "Aucune mise à jour n'a été effectuée.";
            }
...
<body>
    <div class="container">
        ...
        <h2><i class="fas fa-user-edit"></i> Modifier le Locataire</h2>
        ...
        <?php if ($locataire): ?>
            <form action="modifier_locataire.php?id=<?php echo $locataire_id; ?>" method="post" enctype="multipart/form-data">
                
                   <div>
    <label for="genre"><i class="fas fa-venus-mars"></i> Genre*:</label>
    <div class="radio-container">
        <input type="radio" id="genre_m" name="genre" value="M" <?php echo ($locataire['genre'] == 'M') ? 'checked' : ''; ?> required>
        <label for="genre_m"><i class="fas fa-male"></i> M</label>
        <input type="radio" id="genre_mme" name="genre" value="MME" <?php echo ($locataire['genre'] == 'MME') ? 'checked' : ''; ?> required>
        <label for="genre_mme"><i class="fas fa-female"></i> MME</label>
				...
				<div class="flex-container">
                    <div>
                        ...
                        <input type="text" id="nom" name="nom" value="<?php echo htmlspecialchars($locataire['nom'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                    <div>
                        <label for="prenom"><i class="fas fa-user"></i> Prénom*:</label>
                        <input type="text" id="prenom" name="prenom" value="<?php echo htmlspecialchars($locataire['prenom'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                </div>
                ...
                <div class="flex-container">
                    <div>
                        ...
                        <input type="text" id="addresse" name="addresse" value="<?php echo htmlspecialchars($locataire['addresse'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                    <div>
                        <label for="complement"><i class="fas fa-building"></i> Complément:</label>
                        <input type="text" id="complement" name="complement" value="<?php echo htmlspecialchars($locataire['complement'], ENT_QUOTES, 'UTF-8'); ?>">
                    </div>
                </div>
                ...
                <div class="flex-container">
                    <div>
                        ...
                        <input type="text" id="cp" name="cp" value="<?php echo htmlspecialchars($locataire['CP'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                    <div>
                        <label for="ville"><i class="fas fa-city"></i> Ville*:</label>
                        <input type="text" id="ville" name="ville" value="<?php echo htmlspecialchars($locataire['ville'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                </div>
                ...
                <div class="flex-container">
                    <div>
                        ...
                        <input type="tel" id="telephone" name="telephone" value="<?php echo htmlspecialchars($locataire['telephone'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                    <div>
                        <label for="fixe"><i class="fas fa-phone"></i> Téléphone fixe:</label>
                        <input type="tel" id="fixe" name="fixe" value="<?php echo htmlspecialchars($locataire['fixe'], ENT_QUOTES, 'UTF-8'); ?>">
                    </div>
					<div>
                        <label for="email"><i class="fas fa-envelope"></i> Email*:</label>
                        <input type="email" id="email" name="email" value="<?php echo htmlspecialchars($locataire['email'], ENT_QUOTES, 'UTF-8'); ?>" required>
                    </div>
                </div>
                ...
                <div class="flex-container">
                    ...
                    <div>
                        ...
                        <?php if ($locataire['piece_jointe_nom']): ?>
                            <p>Fichier actuel: <?php echo htmlspecialchars($locataire['piece_jointe_nom'], ENT_QUOTES, 'UTF-8'); ?></p>
                        <?php endif; ?>
                    </div>
                </div>
                ...
                <label for="quittance" class="checkbox-container">
                    <input type="checkbox" id="quittance" name="quittance" <?php echo ($locataire['quittance'] == 1) ? 'checked' : ''; ?>>
                    <i class="fas fa-file-alt"></i> Je souhaite une quittance mensuelle
                </label>
            </div>
            <div>
                <label for="rgpd" class="checkbox-container">
                    <input type="checkbox" id="rgpd" name="rgpd" <?php echo ($locataire['rgpd'] == 1) ? 'checked' : ''; ?>>
                ...
                <div class="flex-container">
                    ...
                    <button type="button" onclick="window.location.href='locataires.php'"><i class="fas fa-times"></i> Annuler</button>
				</div>
            </form>
        <?php else: ?>
            <p class="error-message">Aucun locataire trouvé avec cet ID.</p>
        <?php endif; ?>
    </div>
</body>
</html>
